...
def get_tool_tip_position(data):
    global x
    global y
    global z
    global rx
...
while not rospy.is_shutdown():
    ...
    # rospy.Subscriber("/eye_robot/FrameEE", Transform, get_tool_tip_position)
    # positions = []
    ...
    try:
        while not rospy.is_shutdown():
            ...
            if (frame_number % 2 == 1) :
                ...
                # positions.append([x, y, z, rx, ry, rz, rw])
    except KeyboardInterrupt:
        print('interrupted!')
    # pos_path = os.path.join(current_path, f"positions_{folder}.csv")
    # np.savetxt(pos_path, positions, delimiter=",")
    time_record_end = time.perf_counter()
